...
const Payment = () => {
  ...
  const confirmPayment = async (pages) => {
    try {
      ...
      setMessage(
        "Đã xảy ra lỗi khi thực hiện thanh toán. Vui lòng thử lại sau."
      );
      setIsConfirmModalOpen(false);
    }
  };
  ...
  return (
    <div className="max-w-3xl mx-auto border-2 border-gray-300 rounded-lg p-8 mt-10 bg-gray-50">
      ...
      <div className="flex justify-between items-center mt-6">
        <p className="text-lg">
          Số trang còn lại :{" "}
          <span className="font-semibold">{user?.numOfPages}</span>
        </p>
        {/* <button
      ...
      {isConfirmModalOpen && (
        <div className="fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center">
          <div className="bg-white p-6 rounded-lg max-w-md w-full">
            ...
            <p>
              Bạn có chắc chắn muốn thanh toán {pages} trang với tổng số tiền{" "}
              {pages * 1000}đ không?
            </p>
